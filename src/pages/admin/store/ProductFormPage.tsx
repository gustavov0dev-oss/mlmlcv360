...
export default function ProductFormPage() {
  ...
  const productId = pathname.endsWith("nuevo")
    ? null
  ...
  const [commissions, setCommissions] = useState<
    Array<{ level: number; type: string; value: string }>
  >([]);
  const [specs, setSpecs] = useState<SpecRow[]>([]);
  const [attrKey, setAttrKey] = useState("color");
  const [attrType, setAttrType] = useState<"text" | "color" | "image">("text");
  const [draggedVariantIdx, setDraggedVariantIdx] = useState<number | null>(
    null,
  );
  ...
  const load = useCallback(async () => {
    ...
    if (productId) {
      const { data: p } = await database.select<any>("products", {
        select:
          "*, variants:product_variants(*), commissions:product_commissions(*)",
        filter: { id: productId },
        maybeSingle: true,
      });
      if (p) {
        ...
        if ((p as any).specs && typeof (p as any).specs === "object") {
          setSpecs(
            Object.entries((p as any).specs).map(([key, value]) => ({
              key,
              value: String(value),
            })),
          );
  ...
  const save = async () => {
    ...
    const images = media
      ...
      .map((m) => ({ url: m.url, alt: m.alt || form.name }));
    const videos = media
      .filter((m) => m.type === "video")
      .map((m) => ({ url: m.url, thumbnail: m.thumbnail || "" }));
  ...
  const uploadFile = async (file: File, isDigital = false) => {
    ...
    const { success, url, error } = await storage.upload(bucket, path, file, {
      upsert: false,
    });
    if (!success || !url) {
      toast.error(error || "Error subiendo archivo");
      setUploading(false);
      return;
    }
    const publicUrl = url;
    if (isDigital) {
      ...
      setMedia((prev) => [
        ...prev,
        { url: publicUrl, alt: form.name, type: isVideo ? "video" : "image" },
      ]);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <span role="status" className="sr-only">Cargando producto</span>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-card border border-border rounded-xl p-5 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={6}
                  placeholder="Descripción detallada del producto. Soporta sintaxis ![alt](url) para imágenes inline."
                  className="w-full px-4 py-3 bg-muted border border-border rounded-xl text-sm text-foreground outline-none focus:border-primary resize-none"
                />
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-xl p-5 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-3 py-3 bg-muted border border-border rounded-xl text-xs text-muted-foreground hover:text-foreground"
                    >
                      Auto
                    </button>
                  </div>
              ...
              <div>
                <label className="block text-xs font-bold text-foreground mb-1.5">
                  Stock general (sin variantes)
                </label>
                <input
                  type="number"
                  value={form.general_stock}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, general_stock: e.target.value }))
                  }
                  placeholder="0"
                  min="0"
                  className="w-full px-4 py-3 bg-muted border border-border rounded-xl text-sm text-foreground outline-none focus:border-primary"
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Usado cuando el producto no tiene variantes o la variante no
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setForm((p) => ({ ...p, track_stock: e.target.checked }))
                    }
              ...
              <div className="bg-card border border-border rounded-xl overflow-hidden">
                ...
                <div className="p-3">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {media.length} imagen{media.length !== 1 ? "es" : ""} total
                  </p>
                </div>
              </div>
            )}
      ...
      {tab === "media" && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            <label
              className={cn(
                "flex items-center gap-2 px-4 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-bold cursor-pointer hover:bg-primary/90 transition-colors",
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center">
                      <div className="opacity-0 group-hover:opacity-100 absolute top-1 right-1 flex gap-1">
                        <button
                          ...
                          className="w-6 h-6 bg-red-500 rounded-lg flex items-center justify-center text-white hover:bg-destructive"
                        >
                          <X className="w-3 h-3" />
                        </button>
                      </div>
          ...
          <p className="text-xs text-muted-foreground">
            Arrastra las imágenes para reordenar. La primera imagen es la
            portada.
          </p>
        </div>
      )}
              ...
              <div>
                ...
                <div className="flex gap-1">
                  {ATTRIBUTE_TYPES.map((t) => (
                    <button
                      key={t.value}
                      onClick={() => setAttrType(t.value as any)}
                      className={cn(
                        "flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-bold border transition-colors",
                        attrType === t.value
          ...
          {variants.length === 0 ? (
            <div className="text-center py-12 border-2 border-dashed border-border rounded-xl text-muted-foreground">
              ...
              <p className="text-xs mt-1">
                Agrega variantes de color, talla u otros atributos
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {variants.map((v, i) => (
                <div
                  ...
                  onDrop={() => {
                    if (draggedVariantIdx === null || draggedVariantIdx === i)
                      return;
                    const newV = [...variants];
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setVariants((p) =>
                            p.map((x, j) =>
                              j === i
                                ? { ...x, compare_price: e.target.value }
                                : x,
                            ),
                          )
                        }
                        ...
                        <label className="px-3 py-2 bg-primary text-primary-foreground rounded-xl text-xs font-bold cursor-pointer hover:bg-primary/90">
                          ...
                          <input
                            ...
                            onChange={async (e) => {
                              ...
                              const { success, url } = await storage.upload(
                                "products",
                                path,
                                file,
                                { upsert: false },
      ...
      {tab === "commissions" && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-foreground">
                Comisiones MLM por nivel
              </h3>
              <p className="text-xs text-muted-foreground">
                Sobrescribe las comisiones globales para este producto
              </p>
            </div>
            <button
              onClick={() =>
                setCommissions((p) => [
                  ...p,
                  { level: p.length + 1, type: "percentage", value: "" },
                ])
              }
        ...
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Descripción para buscadores (máx. 160 caracteres recomendado)"
              className="w-full px-4 py-3 bg-muted border border-border rounded-xl text-sm text-foreground outline-none focus:border-primary resize-none"
            />
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          <div className="flex items-start gap-3 p-3 bg-primary/8 border border-primary/20 rounded-xl">
            ...
            <div className="text-xs text-primary">
              ...
              <p>
                El archivo se entrega automáticamente al cliente tras confirmar
                el pago.
              </p>
            </div>
          </div>
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, digital_file_url: e.target.value }))
                }
              ...
              <label
                className={cn(
                  "flex items-center gap-2 px-4 py-3 bg-primary text-primary-foreground rounded-xl text-sm font-bold cursor-pointer hover:bg-primary/90",
                  uploading && "opacity-50 cursor-not-allowed",
                )}
              >
                ...
                <input
                  ...
                  disabled={uploading}
                  onChange={(e) =>
                    e.target.files?.[0] && uploadFile(e.target.files[0], true)
                  }
                />
              </label>
            </div>
          ...
          <div>
            <label className="block text-xs font-bold text-foreground mb-1.5">
              URL de demostración gratuita
            </label>
            <input
              value={form.digital_demo_url}
              onChange={(e) =>
                setForm((p) => ({ ...p, digital_demo_url: e.target.value }))
              }
              placeholder="https://... (video, iframe o enlace público)"
              className="w-full px-4 py-3 bg-muted border border-border rounded-xl text-sm text-foreground outline-none focus:border-primary"
            />
            <p className="text-xs text-muted-foreground mt-1">
              El cliente puede ver esta demo <strong>antes</strong> de comprar.
              Acepta MP4, YouTube, Vimeo o cualquier URL embebible.
            </p>
          </div>
          <div>
            <label className="block text-xs font-bold text-foreground mb-1.5">
            ...
            <textarea
              value={form.digital_instructions}
              onChange={(e) =>
                setForm((p) => ({ ...p, digital_instructions: e.target.value }))
              }
              rows={4}
              placeholder="Instrucciones para el cliente tras la compra..."
              className="w-full px-4 py-3 bg-muted border border-border rounded-xl text-sm text-foreground outline-none focus:border-primary resize-none"
            />
          </div>
        </div>
