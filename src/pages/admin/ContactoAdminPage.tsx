...
export default function ContactoAdminPage(){
 ...
 return <StableRegion className="space-y-6 max-w-4xl">
  ...
  {tab==='messages'?<ContactInbox/>:loading?<LoadingRegion className="min-h-[24rem]" />:failed?<p role="alert" className="text-destructive text-sm">No se pudo cargar el contenido. Pulsa Actualizar para reintentar.</p>:section&&<fieldset disabled={saving} className="space-y-5">
   ...
   <div className="flex justify-between items-center border-t border-border pt-4 gap-4 form-actions"><p className="text-xs text-muted-foreground">{dirty.has(tab)?'Cambios pendientes. Guarda para publicarlos.':'Sin cambios pendientes.'}</p><button onClick={save} disabled={saving||!dirty.has(tab)} className="flex items-center gap-2 px-5 py-2.5 bg-primary text-primary-foreground rounded-lg text-sm disabled:opacity-50"><Save className="w-4 h-4"/>Guardar sección</button></div>
  </fieldset>}
 </StableRegion>;
}
