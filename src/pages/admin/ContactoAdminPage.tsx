...
export default function ContactoAdminPage(){
 ...
 return <div className="space-y-6 max-w-4xl">
  ...
  <div className="flex gap-1 p-1 bg-muted rounded-xl overflow-x-auto">{[...contactTabs,{id:'messages' as const,label:'Mensajes'}].map(({id,label})=><button key={id} disabled={saving} onClick={()=>setTab(id)} className={cn('px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap',tab===id?'bg-card text-foreground shadow-sm':'text-muted-foreground')}>{label}{id==='messages'&&notifications.count>0&&<span className="ml-2 bg-primary text-primary-foreground rounded-full px-1.5 text-xs">{notifications.count}</span>}{id!=='messages'&&dirty.has(id)&&' •'}</button>)}</div>
  {tab==='messages'?<ContactInbox/>:loading?<div className="h-48 animate-pulse bg-muted rounded-xl" role="status" aria-label="Cargando contenido"/>:failed?<p role="alert" className="text-destructive text-sm">No se pudo cargar el contenido. Pulsa Actualizar para reintentar.</p>:section&&<fieldset disabled={saving} className="space-y-5">
