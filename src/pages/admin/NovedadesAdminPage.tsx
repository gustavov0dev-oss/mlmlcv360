...
export default function NovedadesAdminPage(){
  ...
  const save=async()=>{
    ...
    setBusy(true);
    try {const data={...form,author:form.author.trim()||user.full_name||user.username||'Equipo editorial',authorAvatar:form.authorAvatar.trim()||user.avatar_url||'',duration:directVideoUrl(form.videoUrl)?form.duration:'',readTime:form.type==='video'?'':readingTime(form.content),slug:newsSlug(form.slug),content:safeNewsHtml(form.content)};const payload={slug:data.slug,status,sort_order:order,data};const result=editing?await database.update<NewsRow>('novedades_posts',editing.id,payload):await database.insert<NewsRow>('novedades_posts',payload);if(result.error)throw new Error(result.error);toast.success(status==='draft'?'Borrador guardado':'Publicación guardada');setForm(null);setEditing(null);await reload();}catch(e){toast.error(String(e).includes('duplicate')?'Ese enlace ya existe. Elige otro.':'No se pudo guardar. Conservamos tus cambios para reintentar.');}finally{setBusy(false);}
  };
  return <div className="space-y-6 max-w-5xl">
    ...
    {loading?<LoadingRegion className="min-h-[24rem]"/>:tab==='page'?<div className="rounded-xl border border-border p-5 space-y-4">
      ...
      <div className="form-actions"><Button disabled={busy||error||!pageForm} onClick={async()=>{setBusy(true);try{const r=await database.upsert('novedades_page',{id:'main',data:pageForm},'id');if(r.error)throw new Error();await reload();setPageForm(null);toast.success('Textos actualizados');}catch{toast.error('No se pudieron guardar los textos');}finally{setBusy(false);}}}>{busy?'Guardando…':'Guardar textos'}</Button></div>
      ...
      </fieldset><div className="flex justify-end gap-2 border-t border-border pt-4 form-actions"><Button variant="outline" onClick={close} disabled={busy||uploads>0}>Cancelar</Button><Button onClick={()=>void save()} disabled={busy||uploads>0}>{busy?'Guardando…':status==='draft'?'Guardar borrador':'Guardar y publicar'}</Button></div>
    </div>:<>
