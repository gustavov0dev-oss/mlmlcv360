...
import { newsSlug, newsPageDefaults, safeMediaUrl, safeNewsHtml, videoEmbed, type NewsRow, type ContentItem } from '@/lib/newsContent';
...
export default function NovedadesAdminPage(){
  ...
  const close=()=>{if(form && !window.confirm('¿Cerrar el editor? Los cambios sin guardar se perderán.'))return;setForm(null);setEditing(null);};
  const save=async()=>{
    if(!form)return;
    if(!form.title.trim()||!newsSlug(form.slug)||!form.category.trim()){toast.error('Completa título, enlace y categoría');return;}
    if(status==='published'&&(!form.excerpt.trim()||!form.content.replace(/<[^>]*>/g,'').trim()||!safeMediaUrl(form.image))){toast.error('Para publicar, añade resumen, contenido y una portada válida');return;}
    ...
    if(form.videoUrl&&!videoEmbed(form.videoUrl)){toast.error('Usa un enlace válido de YouTube o Vimeo');return;}
    setBusy(true);
    try {const data={...form,slug:newsSlug(form.slug),content:safeNewsHtml(form.content)};const payload={slug:data.slug,status,sort_order:order,data};const result=editing?await database.update<NewsRow>('novedades_posts',editing.id,payload):await database.insert<NewsRow>('novedades_posts',payload);if(result.error)throw new Error(result.error);toast.success(status==='draft'?'Borrador guardado':'Publicación guardada');setForm(null);setEditing(null);await reload();}catch(e){toast.error(String(e).includes('duplicate')?'Ese enlace ya existe. Elige otro.':'No se pudo guardar. Conservamos tus cambios para reintentar.');}finally{setBusy(false);}
  };
  return <div className="space-y-6 max-w-5xl">
    ...
    {loading?<LoadingRegion className="min-h-[24rem]"/>:tab==='page'?<div className="rounded-xl border border-border p-5 space-y-4">
      ...
      <div className="grid sm:grid-cols-2 gap-4">{([['title','Título'],['slug','Enlace de la publicación'],['category','Categoría'],['date','Fecha visible'],['author','Autor'],['authorRole','Cargo del autor'],['readTime','Tiempo de lectura'],['duration','Duración del video']] as const).map(([key,label])=><label key={key} className="text-sm space-y-2 block"><span>{label}</span><Input value={form[key]||''} onChange={e=>{set(key,e.target.value);if(key==='title'&&!editing)set('slug',newsSlug(e.target.value));}}/></label>)}</div>
      ...
      {form.type==='video'&&<label className="block text-sm space-y-2"><span>Video de YouTube o Vimeo</span><Input type="url" value={form.videoUrl} onChange={e=>set('videoUrl',e.target.value)} placeholder="https://…"/></label>}
