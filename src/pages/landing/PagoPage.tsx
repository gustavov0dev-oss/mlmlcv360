...
export default function PagoPage() {
 ...
 useEffect(()=>{if(!user)return;let active=true;setLoaded(false);setPending(null);setPlan(null);setSession(null);setContract(null);setError('');
  (async()=>{
  ...
  {!user?planSlug?<GuestPlanCheckout slug={planSlug}/>:<p className="mt-5"><Link to="/login" className="text-primary">Inicia sesión</Link> para continuar.</p>:<div className="mt-6 space-y-6 min-h-[320px]" aria-busy={busy||!loaded}>
   ...
   {returning&&record&&<>
    ...
    {paid?<div role="status" className="space-y-3"><CheckCircle className="w-7 h-7 text-emerald-500"/><p>{isOrder?'Tu pedido está pagado. Puedes seguir su estado desde Mis pedidos.':`Tu membresía está activa.${contract?.paid_until?' Beneficios hasta el '+new Date(contract.paid_until).toLocaleDateString('es-PE')+'.':''}`}</p><Link to={back} className={button}>{isOrder?'Ver mis pedidos':'Ver mi membresía'}</Link></div>:
     ...
     contract?.authorization_unavailable?<div className="space-y-4"><p role="status" className="text-sm text-muted-foreground">{contract.last_error} Tu membresía actual sigue igual.</p><button className={button} disabled={busy} onClick={async()=>{setBusy(true);setError('');try{await invoke('subscription-billing',{action:'cancel',contract_id:contract.id});window.location.assign(`/pago?plan=${encodeURIComponent(contract.plan_slug)}`);}catch(e:any){setError(e.message);setBusy(false);}}}>Descartar y preparar nueva autorización</button></div>:
