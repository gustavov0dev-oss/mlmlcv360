...
import {parseWhatsAppConfig,validContactPhoto,validWhatsAppNumber,whatsappKey,whatsappLink,type WhatsAppConfig} from '@/lib/whatsappContent';

function ContactPhoto({photo}:{photo?:string}){
 const [failed,setFailed]=useState(false);
 useEffect(()=>setFailed(false),[photo]);
 return photo&&validContactPhoto(photo)&&!failed?<img src={photo} alt="" width={40} height={40} className="w-10 h-10 rounded-full object-cover" referrerPolicy="no-referrer" onError={()=>setFailed(true)}/>:<WhatsAppIcon className="w-5 h-5"/>;
}
...
export default function WhatsAppButton(){
 ...
 useEffect(()=>{if(!open)return;close.current?.focus({preventScroll:true});const key=(event:KeyboardEvent)=>{if(event.key==='Escape'){setOpen(false);trigger.current?.focus();}};const outside=(event:PointerEvent)=>{if(!root.current?.contains(event.target as Node))setOpen(false);};document.addEventListener('keydown',key);document.addEventListener('pointerdown',outside);return()=>{document.removeEventListener('keydown',key);document.removeEventListener('pointerdown',outside);};},[open]);
 useEffect(()=>{
  if(!open)return;
 ...
 return <div ref={root} className={cn('fixed z-30 bottom-[max(1.25rem,env(safe-area-inset-bottom))]',config.position==='left'?'left-4 sm:left-6':'right-4 sm:right-6')}>
  {open&&<section id="whatsapp-contacts" aria-labelledby="whatsapp-title" className={cn('absolute bottom-[4.5rem] w-[min(360px,calc(100vw-2rem))] max-h-[calc(100dvh-7rem)] flex flex-col bg-card border border-border rounded-2xl shadow-2xl overflow-hidden',config.position==='left'?'left-0':'right-0')}>
   <header className="bg-[#075e54] text-white p-5 shrink-0"><div className="flex items-center gap-3"><WhatsAppIcon className="w-6 h-6 shrink-0"/><h2 id="whatsapp-title" className="flex-1 min-w-0 font-semibold text-base leading-snug break-words">{config.title}</h2><button ref={close} onClick={()=>{setOpen(false);trigger.current?.focus();}} aria-label="Cerrar contactos de WhatsApp" className="p-1 rounded-md hover:bg-white/10 focus-visible:outline focus-visible:outline-2"><X className="w-5 h-5"/></button></div><p className="mt-2 text-sm text-white/80 leading-relaxed break-words">{config.description}</p></header>
   <div className="overflow-y-auto p-3 space-y-2"><p className="px-2 py-1 text-xs text-muted-foreground">Selecciona un contacto</p>{contacts.map(row=><a key={row.id} href={whatsappLink(row)} target="_blank" rel="noopener noreferrer" aria-label={`Abrir WhatsApp con ${row.name} en una pestaña nueva`} className="group flex items-center gap-3 p-3 rounded-xl border border-border/60 hover:border-emerald-500/40 hover:bg-emerald-500/5 focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-500 transition-colors"><span className="w-10 h-10 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 shrink-0 grid place-items-center"><ContactPhoto photo={row.photo}/></span><span className="flex-1 min-w-0"><span className="block text-sm font-semibold text-foreground break-words">{row.name}</span>{row.description&&<span className="block text-xs text-muted-foreground mt-1 break-words">{row.description}</span>}</span><ArrowUpRight className="w-4 h-4 text-muted-foreground group-hover:text-emerald-600 shrink-0"/></a>)}</div>
