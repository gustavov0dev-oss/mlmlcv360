...
const errorText:Record<string,string>={smtp_port_blocked:'El alojamiento actual bloquea ese puerto SMTP. Revisa en Correos si tu proveedor admite SSL/TLS en el puerto 465.',email_not_configured:'Completa la configuración SMTP en Configuración → Correos. Tu borrador está guardado.',provider_rejected:'El servicio de correo rechazó el envío. Revisa el remitente y la configuración antes de reintentar.',delivery_unknown:'No se pudo confirmar el envío SMTP. Revisa el registro de tu proveedor antes de reenviar para evitar duplicados.',delivery_record_pending:'El correo fue aceptado, pero falta confirmar el registro. Revisa el historial y el registro del proveedor antes de reenviar.',manual_review_required:'Este envío necesita revisión antes de volver a enviarlo. No crees una copia hasta confirmar si llegó.',sending:'El envío sigue en curso. Espera un minuto antes de verificarlo.',reply_changed_or_busy:'La respuesta cambió o está en proceso. Vuelve a abrir el mensaje.'};
...
function ContactThread({id,onBack}:{id:string;onBack:()=>void}){
 ...
 const checkConfiguration=useCallback(async(manual=false)=>{
  ...
  try{
   ...
   const result=data.detail||(data.configured?'Conexión SMTP comprobada.':'Completa los datos SMTP en Configuración → Correos.');
 ...
 return <div className="space-y-5">
  ...
  <div className={cn('border rounded-xl p-4 text-sm space-y-3',configured===true?'bg-emerald-500/10 border-emerald-500/20':'bg-amber-500/10 border-amber-500/20')} aria-busy={checking}>
   ...
   <Link to="/dashboard/admin?module=correos" className="inline-block text-primary underline text-sm">Abrir configuración SMTP</Link>
  </div>
  {notice&&<div role="alert" className="border border-amber-500/30 rounded-xl p-4 text-sm">{notice}</div>}
  {canCompose?<section className="bg-card border border-border rounded-xl p-5 sm:p-6 space-y-4"><div><h3 className="font-semibold inline-flex items-center gap-2"><Reply className="w-4 h-4 text-primary"/>Responder</h3><p className="text-sm text-muted-foreground mt-1">La respuesta se enviará a <strong>{message.email}</strong>.</p></div><fieldset disabled={busy} className="space-y-4"><label className="block text-sm font-medium">Asunto<input value={subject} maxLength={250} onChange={event=>{setSubject(event.target.value);setDirty(true);}} className={`${inputClass} mt-1.5`}/></label><div className={busy?'pointer-events-none opacity-60':''}><RichTextEditor editable={!busy} value={html} onChange={value=>{setHtml(value);setDirty(true);}} minHeight={220}/></div><div className="flex flex-wrap items-center justify-between gap-3 form-actions"><span className="text-xs text-muted-foreground">{dirty?'Cambios sin guardar':history.some(reply=>reply.id===draftId)?'Borrador guardado':'Nueva respuesta'}</span><div className="flex gap-2 ml-auto"><button onClick={persist} disabled={busy||!subject.trim()} className="inline-flex items-center gap-2 border border-border rounded-lg px-4 py-2 text-sm disabled:opacity-50"><Save className="w-4 h-4"/>Guardar borrador</button><button onClick={()=>send()} disabled={busy||configured!==true||!plainText(html)} className="inline-flex items-center gap-2 bg-primary text-primary-foreground rounded-lg px-4 py-2 text-sm disabled:opacity-50">{busy?<RefreshCw className="w-4 h-4 animate-spin"/>:<Send className="w-4 h-4"/>}Enviar respuesta</button></div></div></fieldset></section>:<p className="text-sm text-muted-foreground inline-flex gap-2"><CheckCheck className="w-4 h-4"/>Hay un envío pendiente de verificar. Si no se confirmó, revisa el registro de tu proveedor SMTP antes de reenviar.</p>}
 </div>;
}
