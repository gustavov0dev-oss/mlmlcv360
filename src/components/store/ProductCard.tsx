  ...
  return (
    <div
      ...
      className={cn(
        ...
        // Mobile: separated by border/divider (handled by parent grid gap + thin border)
        'sm:border-0 border border-border/30 rounded-xl sm:rounded-none',
        isComparing && 'ring-2 ring-primary ring-offset-1 rounded-xl sm:rounded-xl'
      )}
    >
      {/* ── IMAGE ── */}
      ...
      {/* ── INFO ── */}
      <div className="pt-2 pb-2 px-0.5 flex flex-col gap-0.5 sm:px-0 px-1.5">

        {/* Category label */}
        {product.category && (
          <span className="text-[9px] font-black tracking-widest uppercase text-muted-foreground/50 truncate leading-none">
            {(product.category as any).name}
          </span>
        )}

        {/* Name */}
        <h3 className="text-[13px] leading-snug line-clamp-2 text-foreground">
          {product.name}
        </h3>
