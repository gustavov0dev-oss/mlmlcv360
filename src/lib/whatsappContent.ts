export interface WhatsAppContact {id:string;name:string;description:string;number:string;message:string;enabled:boolean}
...
export function validateWhatsAppConfig(config:WhatsAppConfig){
 ...
 for(const row of config.contacts){if(!row.name.trim()||row.name.length>70||row.description.length>120||row.message.length>1000||!validWhatsAppNumber(row.number))return 'Cada contacto necesita nombre y número con código de país (8 a 15 dígitos). Revisa también los límites de texto.';}
 return null;
}
